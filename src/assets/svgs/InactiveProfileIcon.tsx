import * as React from 'react';
import Svg, {SvgProps, G, Path, Defs, ClipPath, Rect} from 'react-native-svg';

const InactiveProfileIcon = (props: SvgProps) => (
  <Svg width={30} height={30} viewBox="0 0 30 30" fill="none" {...props}>
    <G clipPath="url(#clip0_27_1070)">
      <Path
        d="M3.75 15C3.75 16.4774 4.04099 17.9403 4.60636 19.3052C5.17172 20.6701 6.00039 21.9103 7.04505 22.955C8.08971 23.9996 9.3299 24.8283 10.6948 25.3936C12.0597 25.959 13.5226 26.25 15 26.25C16.4774 26.25 17.9403 25.959 19.3052 25.3936C20.6701 24.8283 21.9103 23.9996 22.955 22.955C23.9996 21.9103 24.8283 20.6701 25.3936 19.3052C25.959 17.9403 26.25 16.4774 26.25 15C26.25 13.5226 25.959 12.0597 25.3936 10.6948C24.8283 9.3299 23.9996 8.08971 22.955 7.04505C21.9103 6.00039 20.6701 5.17172 19.3052 4.60636C17.9403 4.04099 16.4774 3.75 15 3.75C13.5226 3.75 12.0597 4.04099 10.6948 4.60636C9.3299 5.17172 8.08971 6.00039 7.04505 7.04505C6.00039 8.08971 5.17172 9.3299 4.60636 10.6948C4.04099 12.0597 3.75 13.5226 3.75 15Z"
        stroke="#CCCCCC"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M11.25 12.5C11.25 13.4946 11.6451 14.4484 12.3483 15.1517C13.0516 15.8549 14.0054 16.25 15 16.25C15.9946 16.25 16.9484 15.8549 17.6517 15.1517C18.3549 14.4484 18.75 13.4946 18.75 12.5C18.75 11.5054 18.3549 10.5516 17.6517 9.84835C16.9484 9.14509 15.9946 8.75 15 8.75C14.0054 8.75 13.0516 9.14509 12.3483 9.84835C11.6451 10.5516 11.25 11.5054 11.25 12.5Z"
        stroke="#CCCCCC"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M7.70996 23.5613C8.01935 22.5315 8.65242 21.629 9.51527 20.9875C10.3781 20.346 11.4248 19.9997 12.5 20H17.5C18.5765 19.9996 19.6245 20.3467 20.4879 20.9897C21.3514 21.6327 21.9843 22.5372 22.2925 23.5688"
        stroke="#CCCCCC"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </G>
    <Defs>
      <ClipPath id="clip0_27_1070">
        <Rect width={30} height={30} fill="white" />
      </ClipPath>
    </Defs>
  </Svg>
);
export default InactiveProfileIcon;
